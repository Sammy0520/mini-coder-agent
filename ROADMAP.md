# Mini Coder Agent 能力路线图

本文档用于指导 Mini Coder Agent 后续实现。路线图按能力之间的依赖关系排序，不绑定具体日期。每个阶段都给出目标能力、实现任务和验收标准；开发时按顺序推进，并在完成后勾选对应项目。

## 1. 项目目标

Mini Coder Agent 的目标不是复刻 Codex 的全部产品能力，而是实现一个规模可控、核心逻辑可检查、能完成真实编程任务的小型 Coding Agent。

达到考核提交标准时，它应当具备以下特征：

1. **可运行**：能够连接真实模型，完成读取、修改、执行测试和总结的完整闭环。
2. **可恢复**：任务中断后可以从持久化 Session 恢复，不丢失有效上下文，也不盲目重复副作用。
3. **可审查**：文件修改有统一 Diff、审批记录、快照和冲突检测。
4. **可验证**：本地能够判断任务是否经过有效验证，而不是仅相信模型声称“已经完成”。
5. **可控制**：模型调用、命令执行、运行时间、输出大小和重试次数都有明确上限。
6. **可解释**：关键状态、工具调用、错误、重试和最终结果可以通过结构化事件查看。
7. **有安全边界**：工作区、敏感文件、命令风险、日志脱敏和人工审批共同构成分层防护。
8. **有评测证据**：通过自动化测试、真实模型 Eval、CI 和演示任务证明能力，而不只罗列功能。

## 2. 实现原则

- 不依赖 LangChain、AutoGen、OpenAI Agents SDK 等 Agent 框架；核心循环由项目自行实现。
- 模型负责推理和选择下一步，本地代码负责权限、状态、执行、持久化和客观验证。
- 先保证单 Agent 闭环可靠，再考虑扩展工具数量或更复杂的 Agent 结构。
- 所有外部副作用必须可识别、可审批、可记录；无法可靠分类时采用更保守的策略。
- API Key 不写入配置、Session、日志、Eval 报告或 Git 仓库。
- 每个里程碑都必须附带自动化测试，并尽量形成一个职责单一的提交。
- 真实模型暴露的兼容性问题应匿名化并固化为离线回归测试。
- README 中的能力描述必须与已经实现和验证的行为一致。

## 3. 能力依赖关系

```mermaid
flowchart TD
    A[当前可运行基线] --> B[真实模型基线验收]
    B --> C[Session 持久化与恢复]
    C --> D[ChangeTracker / Diff / Undo]
    C --> E[任务状态与验证闭环]
    D --> E
    E --> F[重试、预算、命令风险与脱敏]
    F --> G[项目理解与工具体验增强]
    G --> H[Eval、CI、演示与发布]

    T[单元测试与安全回归] -.贯穿所有阶段.-> B
    T -.-> C
    T -.-> D
    T -.-> E
    T -.-> F
    T -.-> G
    T -.-> H
```

如果时间有限，优先完成 B～F，再完成 H。阶段 G 属于竞争力增强项，可以根据剩余时间裁剪。

---

## 4. 阶段 A：当前可运行基线

### 目标能力

形成一个不依赖 Agent 框架、可以调用模型和本地工具、具备基础安全控制的最小完整 Agent。

### 已完成

- [x] 建立与厂商无关的 `ModelClient` 抽象。
- [x] 实现 `OpenAICompatibleClient`。
- [x] 支持 Responses 与 Chat Completions 两种 wire API。
- [x] 支持 provider TOML、环境变量和本地忽略的 `auth.json`。
- [x] 实现 Agent 工具循环和工具结果回传。
- [x] 实现文件列举、读取、搜索、写入和精确编辑工具。
- [x] 实现本地命令执行工具。
- [x] 实现工具参数校验和未知参数拒绝。
- [x] 实现工作区路径限制和常见敏感文件过滤。
- [x] 实现默认人工审批与受控目录 `--auto` 模式。
- [x] 实现最大步骤数和连续重复工具调用检测。
- [x] 实现命令超时、输出截断和模型密钥环境变量隔离。
- [x] 实现基础上下文压缩，并保持工具调用消息配对合法。
- [x] 实现可选 JSONL 事件日志。
- [x] 建立 24 项离线单元测试。
- [x] 初始化 Git，完成公开仓库和首轮推送。

### 当前限制

- [ ] 尚未在仓库中保存一份经过脱敏的真实模型端到端运行记录。
- [ ] 任务状态主要存在于进程内，退出后不能可靠恢复。
- [ ] 写操作没有面向用户的统一 Diff 和 Session 级 Undo。
- [ ] 本地尚未区分“模型声称完成”和“经过测试验证完成”。
- [ ] API 错误分类、有限重试和运行预算仍不完整。
- [ ] 命令审批尚未形成明确的风险等级模型。
- [ ] 尚未建立独立 Eval 框架和 GitHub Actions。

---

## 5. 阶段 B：真实模型基线验收

### 目标能力

证明当前核心循环不仅能通过假模型测试，也能在真实 aicode007 Responses 服务上完成编程闭环；同时为后续重构保存稳定基线。

### 为什么先做

Session、Diff 和状态机都会修改 AgentRunner 的关键路径。在重构之前先记录真实行为，可以区分“原本就存在的兼容性问题”和“后续改造引入的回归”。

### 实现任务

- [ ] 准备一个可重复重置、可以安全丢弃的真实演示工作区。
- [ ] 设计一个只读代码分析任务。
- [ ] 设计一个单文件 Bug 修复并运行测试的任务。
- [ ] 设计一个测试首次失败、模型需要继续修正的任务。
- [ ] 使用当前 aicode007 Responses 配置完成至少一次端到端运行。
- [ ] 检查多轮 Responses output、function call 与 function result 是否正确重放。
- [ ] 检查真实模型是否产生多工具调用、空文本、额外参数或格式异常。
- [ ] 将真实兼容性问题转化成不需要 API Key 的响应解析 fixture 和单元测试。
- [ ] 保存一份脱敏运行记录，记录任务、关键步骤、修改文件和测试结果。
- [ ] 确认运行记录不含 API Key、Authorization header 或本地隐私路径。

### 建议产物

```text
examples/
└── demo_project/

docs/
└── runs/
    └── baseline-responses-run.md

tests/
└── fixtures/
    └── responses/
```

运行记录只保存对理解 Agent 行为有用的内容，不提交完整敏感请求头或未经检查的原始日志。

### 验收标准

- [ ] Agent 能完成“读取 → 修改 → 测试 → 总结”的真实闭环。
- [ ] 修改内容与任务一致，没有工作区外访问和明显无关改动。
- [ ] 测试失败时不会直接宣布成功。
- [ ] 最终结果明确列出修改内容和真实测试结果。
- [ ] 至少一个真实响应样例已经转化为离线回归测试。
- [ ] 完整离线测试仍然通过。

---

## 6. 阶段 C：Session 持久化与恢复

### 目标能力

让每次 Agent 运行成为一个可持久化、可检查、可恢复的 Session。正常退出、Ctrl+C、模型错误或进程崩溃后，能够安全恢复有效上下文。

### 6.1 Session Schema

- [ ] 为 Session 定义独立、版本化的数据结构。
- [ ] 增加 `schema_version`，为未来迁移保留接口。
- [ ] 保存 `session_id`、创建时间、更新时间和工作区。
- [ ] 保存原始任务、当前状态、步骤编号和停止原因。
- [ ] 保存必要的 provider/model 配置摘要，但不保存 API Key。
- [ ] 保存对话消息、Responses output items 和工具结果。
- [ ] 保存审批决定、修改记录引用和验证状态。
- [ ] 保存最后一个可诊断错误的脱敏摘要。

建议的逻辑结构：

```text
Session
├── metadata
│   ├── schema_version
│   ├── session_id
│   ├── created_at / updated_at
│   ├── workspace
│   └── task
├── runtime
│   ├── status
│   ├── current_step
│   ├── stop_reason
│   └── verification_state
├── conversation
│   ├── messages
│   └── provider_state
├── tool_executions
├── approvals
├── changes
└── result / last_error
```

### 6.2 SessionStore

- [ ] 新建职责单一的 `SessionStore`。
- [ ] 使用临时文件加原子替换保存 Session。
- [ ] 保存前完成 JSON 序列化和 schema 校验。
- [ ] 加载时识别文件损坏、缺字段和不支持的版本。
- [ ] Session 文件默认放在工作区内被 Git 忽略的位置，例如 `.mini-coder/sessions/`。
- [ ] 限制 Session 文件权限的最佳实践，并在文档中说明其可能包含代码和命令输出。
- [ ] 对写入失败、磁盘空间不足和目标目录不可用给出清晰错误。

### 6.3 运行状态机

- [ ] 定义集中管理的状态枚举，避免散落字符串。
- [ ] 至少支持以下状态：

```text
created
running
waiting_for_approval
interrupted
failed
completed_verified
completed_unverified
cancelled
```

- [ ] 约束合法状态转换。
- [ ] 在模型请求、工具请求、审批、工具完成和最终结束后保存关键检查点。
- [ ] Ctrl+C 时将 Session 标记为 `interrupted` 或 `cancelled`，而不是留下 `running`。

### 6.4 工具执行状态与幂等恢复

- [ ] 为每个工具调用分配稳定的 `tool_execution_id`。
- [ ] 记录工具状态：`requested`、`approved`、`running`、`completed`、`failed`、`uncertain`。
- [ ] 已完成工具调用恢复后不得自动重复执行。
- [ ] 对崩溃时处于 `running` 的有副作用工具标记为 `uncertain`。
- [ ] 对 `uncertain` 写操作检查文件状态或要求用户确认。
- [ ] 对 `uncertain` 外部副作用命令默认不自动重放。
- [ ] 只读、可安全重复的工具可以在明确规则下重试。

### 6.5 CLI 恢复入口

- [ ] 增加 `--resume <session-id-or-path>`。
- [ ] 恢复前显示任务、工作区、最后状态和未完成操作摘要。
- [ ] 工作区不存在或路径不一致时停止并提示用户。
- [ ] 恢复时重新校验当前配置和 API Key 来源。
- [ ] 不允许通过 Session 内容覆盖当前安全策略。
- [ ] 可选：增加 `--list-sessions` 和 Session 摘要列表。

### 测试要求

- [ ] 正常保存和加载。
- [ ] 多轮工具调用恢复。
- [ ] Responses output 与 function result 配对恢复。
- [ ] Ctrl+C 状态保存。
- [ ] 损坏 JSON。
- [ ] 不支持的 schema 版本。
- [ ] 工作区丢失。
- [ ] 已完成写操作不重复执行。
- [ ] 不确定副作用需要重新确认。
- [ ] Session 序列化结果不包含测试用 API Key。

### 验收标准

- [ ] 一个进行到一半的真实任务可以中断并恢复。
- [ ] 恢复后模型获得合法且足够的上下文。
- [ ] 不会无条件重复已经完成或结果不确定的副作用。
- [ ] Session 文件损坏时安全失败，不覆盖原文件。
- [ ] Session 文件不包含任何模型服务凭据。
- [ ] CLI、事件日志和 Session 最终状态保持一致。

---

## 7. 阶段 D：ChangeTracker、Diff、快照与 Undo

### 目标能力

让 Agent 的每次文件修改都可预览、可追踪、可撤销，并能识别用户或其他进程造成的并发修改。

### 7.1 ChangeTracker

- [ ] 新建 `ChangeTracker`，与具体文件工具解耦。
- [ ] 每次修改记录路径、工具调用 ID、修改前后 hash 和时间戳。
- [ ] 保存修改前快照或可恢复备份。
- [ ] 将修改记录关联到当前 Session。
- [ ] 同一文件多次修改时保留有序历史。

建议记录：

```text
path
tool_execution_id
before_hash
after_hash
before_snapshot
unified_diff
created_at
undo_status
```

### 7.2 Unified Diff

- [ ] 为 `write_file` 和 `edit_file` 生成统一 Diff。
- [ ] 新文件、删除内容和普通修改使用一致格式。
- [ ] 审批前显示文件路径、增删行数和 Diff。
- [ ] 对过大的 Diff 进行截断，并明确标记截断情况。
- [ ] 最终回答汇总所有修改文件及增删统计。

### 7.3 原子写入与冲突检测

- [ ] 写入前记录模型读取或生成修改时对应的文件 hash。
- [ ] 实际写入前再次检查当前文件 hash。
- [ ] 文件已被外部修改时拒绝盲目覆盖，并返回冲突错误。
- [ ] 使用同目录临时文件和原子替换，避免半写入文件。
- [ ] 正确保留文本编码和换行风格。
- [ ] 对二进制文件、大文件和符号链接采用明确策略。

### 7.4 Undo

- [ ] 支持撤销当前 Session 的最后一次文件修改。
- [ ] 可选：支持撤销当前 Session 的全部文件修改。
- [ ] Undo 前确认当前 hash 仍等于 Agent 修改后的 `after_hash`。
- [ ] 当前文件已被再次修改时报告冲突，不覆盖用户新内容。
- [ ] Undo 本身也写入 Session 和事件日志。
- [ ] 明确说明 Undo 只处理受 ChangeTracker 管理的文件修改，不承诺撤销任意 shell 命令副作用。

### 测试要求

- [ ] 新文件 Diff。
- [ ] 普通修改 Diff。
- [ ] 删除多行 Diff。
- [ ] 用户拒绝后文件不变。
- [ ] 原子写入失败时原文件不变。
- [ ] 外部修改冲突。
- [ ] 单次 Undo。
- [ ] 多次修改按逆序 Undo。
- [ ] Undo 冲突不覆盖用户修改。
- [ ] Windows 与 Unix 换行处理。

### 验收标准

- [ ] 用户批准修改前能够理解将发生什么变化。
- [ ] 所有内置文件写操作都进入 ChangeTracker。
- [ ] Session 能展示完整修改历史。
- [ ] 安全 Undo 不会覆盖用户后续改动。
- [ ] 最终结果包含与真实文件一致的变更汇总。

---

## 8. 阶段 E：任务状态、验证闭环与最终报告

### 目标能力

将“模型停止调用工具”与“任务客观完成”分离。本地 Runner 维护可验证事实，并准确报告成功、未验证、失败、拒绝或中断。

### 8.1 任务阶段

- [ ] 在 Session 中记录轻量级任务阶段：`analyze`、`implement`、`verify`、`summarize`。
- [ ] 阶段用于状态和提示，不把模型限制成僵硬工作流。
- [ ] 用户只要求分析时允许不进入 implement 和 verify。

### 8.2 验证状态

- [ ] 记录是否发生过文件修改。
- [ ] 记录验证命令、退出码、执行时间和输出摘要。
- [ ] 文件修改后将之前的验证结果标记为失效。
- [ ] 修改后再次成功验证才可标记 `completed_verified`。
- [ ] 用户明确禁止运行命令时允许 `completed_unverified`。
- [ ] 测试失败时不得标记 verified。
- [ ] 达到步骤、时间或预算上限时保留未完成状态和原因。

### 8.3 本地完成规则

- [ ] 模型声称完成但测试失败时，本地状态仍为失败或未验证。
- [ ] 模型声称测试通过时，以最近一次真实命令结果为准。
- [ ] 没有代码修改的分析任务可以按自身验收规则正常完成。
- [ ] 用户拒绝关键操作后区分 `denied`、替代方案完成和普通失败。

### 8.4 最终报告

- [ ] 最终报告包含任务状态和停止原因。
- [ ] 列出修改文件及 Diff 统计。
- [ ] 列出实际执行的验证命令和结果。
- [ ] 列出未解决错误、未验证项目和用户拒绝项。
- [ ] 报告模型调用数、工具调用数、重试数和总耗时。
- [ ] CLI 退出码与最终状态一致。

建议最终摘要结构：

```text
Outcome
Changed files
Validation
Unresolved items
Run statistics
```

### 测试要求

- [ ] 修改后未测试得到 `completed_unverified`。
- [ ] 测试通过后得到 `completed_verified`。
- [ ] 测试通过后再次修改会使验证失效。
- [ ] 测试失败不能被模型文本覆盖。
- [ ] 只读任务不要求无意义测试。
- [ ] 用户拒绝写入时状态准确。
- [ ] 步骤上限、重复调用和 Ctrl+C 的状态准确。

### 验收标准

- [ ] 最终状态由本地事实决定，不只依赖模型自然语言。
- [ ] 用户可以判断代码是否真的经过验证。
- [ ] Session、日志、CLI 退出码和最终报告一致。

---

## 9. 阶段 F：错误恢复、运行预算、命令风险与脱敏

### 目标能力

让 Agent 在真实网络和本地执行环境中可预测地失败、有限恢复，并防止错误重试、无限输出或高风险命令绕过控制。

### 9.1 模型 API 错误分类

- [ ] 定义统一错误类型：认证、权限、限流、超时、服务端错误、请求错误、响应解析错误。
- [ ] 网络超时允许有限重试。
- [ ] HTTP 429 尊重 `Retry-After`，否则指数退避。
- [ ] HTTP 500、502、503、504 允许有限重试。
- [ ] HTTP 401、403 不重试，直接报告认证或权限问题。
- [ ] HTTP 400 不盲目重试，输出脱敏兼容性诊断。
- [ ] 响应解析错误最多谨慎重试一次，并保留匿名化诊断。
- [ ] 重试使用抖动并设置硬上限。

### 9.2 运行预算

- [ ] 保留最大步骤数。
- [ ] 增加最大总运行时间。
- [ ] 增加最大模型调用次数和工具调用次数。
- [ ] 增加单次与累计工具输出预算。
- [ ] 如果 provider 返回 usage，记录输入、输出和总 token。
- [ ] provider 不返回 usage 时明确标记未知，不估造数据。
- [ ] 达到预算时安全停止并保存可恢复 Session。

建议 CLI 配置：

```text
--max-steps
--max-seconds
--max-model-calls
--max-tool-calls
--max-tool-output
```

### 9.3 命令风险模型

- [ ] 定义命令风险等级：`read_only`、`workspace_write`、`external_effect`、`dangerous`、`unknown`。
- [ ] 已知只读命令可以使用较低确认等级。
- [ ] 修改 Git 状态、安装依赖、网络上传等命令明确标记副作用。
- [ ] 删除、移动、覆盖和无法理解的复合命令采用更高风险等级。
- [ ] 无法可靠分类时默认为 `unknown`，不自动批准。
- [ ] 审批提示说明命令、工作目录、风险等级和预期副作用。
- [ ] 不宣称通用 shell 静态分析可以构成完整沙箱。

### 9.4 子进程生命周期

- [ ] timeout 时终止子进程。
- [ ] 尽可能终止对应的子进程树。
- [ ] Ctrl+C 后不留下由 Agent 启动的后台测试进程。
- [ ] 保存退出码、超时状态和截断状态。
- [ ] 保持模型服务 API Key 不进入子进程环境。

### 9.5 统一脱敏

- [ ] 实现集中式 `redact_sensitive_text()` 或等价组件。
- [ ] 对异常、日志、请求诊断、命令输出和最终报告统一脱敏。
- [ ] 识别常见 Bearer token、API Key 和 URL token 参数。
- [ ] 防止 SDK 异常对象泄露请求头。
- [ ] 脱敏前的原始敏感内容不得写入普通日志。
- [ ] 使用假密钥建立回归测试。

### 9.6 结构化事件

- [ ] 固定核心事件类型：

```text
run_started
model_request_started
model_response_received
retry_scheduled
tool_call_requested
tool_call_approved
tool_call_denied
tool_call_completed
context_compacted
verification_completed
run_completed
run_failed
run_cancelled
```

- [ ] 每个事件包含 run/session ID、step、时间戳和持续时间等必要字段。
- [ ] 事件 schema 版本化。
- [ ] 日志写入失败不能破坏主任务，但必须给出可见警告。

### 测试要求

- [ ] 429、500、503、超时和网络错误的有限重试。
- [ ] 400、401、403 不重试。
- [ ] `Retry-After` 解析。
- [ ] 达到时间、调用次数和输出预算。
- [ ] 未知命令需要确认。
- [ ] Ctrl+C 和 timeout 清理子进程。
- [ ] 日志和异常中测试用假密钥被脱敏。

### 验收标准

- [ ] 只有可恢复错误会重试，并且重试有硬上限。
- [ ] 达到预算后任务可预测地停止并保存状态。
- [ ] 命令审批能向用户说明主要风险。
- [ ] API Key 不会通过日志、错误或子进程环境泄露。

---

## 10. 阶段 G：项目理解与工具体验增强

> 本阶段属于竞争力增强项。阶段 B～F 完成后，Agent 已具备核心生产闭环；本阶段用于提高复杂仓库任务的成功率。

### 目标能力

让 Agent 更快理解真实项目，减少无意义文件读取、重复搜索和不必要修改。

### 10.1 项目入口识别

- [ ] 启动时生成轻量级工作区概览，而不是读取所有文件。
- [ ] 识别常见项目清单：`pyproject.toml`、`package.json`、`Cargo.toml`、`pom.xml` 等。
- [ ] 识别测试目录和常用验证命令候选。
- [ ] 识别仓库内明确的 Agent/开发说明文件，并通过本地策略控制其优先级。
- [ ] 跳过 `.git`、虚拟环境、依赖目录、构建产物和大文件。

### 10.2 更稳定的搜索与读取

- [ ] 搜索结果包含文件、行号和截断信息。
- [ ] 大文件支持按行范围读取。
- [ ] 工具结果支持分页或继续读取，而不是一次返回全部内容。
- [ ] 同一步骤重复读取相同内容时给模型提示。
- [ ] 搜索无结果时区分“确实没有”和“结果被过滤”。

### 10.3 Git 感知

- [ ] 提供受控的 Git 状态摘要。
- [ ] 区分任务开始前已有改动与 Agent 新改动。
- [ ] 最终报告不把用户原有改动归因给 Agent。
- [ ] 检测文件是否在任务期间被外部修改。
- [ ] 默认不自动 commit、push、reset 或清理用户工作区。

### 10.4 工具调用效率

- [ ] 记录重复、失败和无效工具调用指标。
- [ ] 对同参数连续调用继续保留硬停止规则。
- [ ] 对不同参数但实质重复的读取提供轻量提示。
- [ ] 根据真实 Eval 判断是否需要批量读取或批量搜索工具，避免仅为工具数量增加复杂度。

### 验收标准

- [ ] 在多文件演示项目中能够定位正确入口和测试命令。
- [ ] 不会扫描虚拟环境或依赖目录污染上下文。
- [ ] 能区分用户原有改动和 Agent 产生的改动。
- [ ] 相比阶段 B 的基线，平均无效工具调用不增加，最好明显下降。

---

## 11. 阶段 H：Eval、CI、演示与发布

### 目标能力

用可重复的证据证明 Agent 的正确性、可靠性和安全边界，并形成适合考核展示的仓库状态。

### 11.1 Eval 框架

- [ ] 建立与普通单元测试分离的 Eval runner。
- [ ] 每个 Eval 使用可重置的独立工作区。
- [ ] Eval 定义任务文本、初始文件、验证命令和评分规则。
- [ ] 支持假模型确定性 Eval。
- [ ] 支持显式开启的真实模型 Eval，默认不在 CI 运行。
- [ ] 真实 Eval 失败时保留脱敏 Session 和报告用于诊断。

至少包含以下场景：

- [ ] 单文件边界 Bug。
- [ ] 多文件接口修改。
- [ ] 测试首次失败后继续修复。
- [ ] 只读代码分析，不产生写操作。
- [ ] 工作区逃逸或敏感文件访问被拒绝。
- [ ] Session 中断后恢复。
- [ ] Diff 审批被拒绝后安全退出或调整方案。
- [ ] Undo 冲突不覆盖用户改动。
- [ ] API 429 或暂时性失败后有限恢复。
- [ ] 长命令输出被截断但保留关键诊断。

### 11.2 Eval 指标

- [ ] 是否完成任务。
- [ ] 是否经过有效验证。
- [ ] 测试是否通过。
- [ ] 是否发生工作区外访问。
- [ ] 是否存在无关修改。
- [ ] 模型调用数和工具调用数。
- [ ] 重试数和总耗时。
- [ ] 修改文件数和 Diff 大小。
- [ ] token usage（仅在 provider 实际返回时记录）。
- [ ] 最终状态和停止原因。

### 11.3 多文件演示项目

- [ ] 建立一个比当前 discount 示例更真实、但仍可快速理解的多文件项目。
- [ ] 任务需要搜索调用关系、修改多个文件并运行测试。
- [ ] 包含一个可展示“首次验证失败后继续修复”的路径。
- [ ] 项目可在每次运行前一键恢复到初始状态。
- [ ] 演示总时长控制在约两分钟，避免长时间等待掩盖核心能力。

### 11.4 GitHub Actions

- [ ] 在 CI 中安装项目并运行全部离线测试。
- [ ] 至少覆盖 Windows 和 Ubuntu。
- [ ] 至少覆盖 Python 3.11 和 3.12。
- [ ] CI 不保存或请求真实模型 API Key。
- [ ] 增加打包或可编辑安装检查。
- [ ] 增加敏感文件误提交检查。
- [ ] 可选：运行确定性的假模型端到端 Eval。

### 11.5 文档与展示

- [ ] README 在前 30 秒内说明项目目标和差异点。
- [ ] 加入简洁架构图。
- [ ] 加入最小安装和运行命令。
- [ ] 加入真实运行示例或终端录屏/GIF。
- [ ] 加入 Eval 结果表和 CI badge。
- [ ] 加入设计决策与安全边界。
- [ ] 加入已知限制，避免声称尚未实现的能力。
- [ ] 检查 `README.md` 与提交要求使用的 `README.txt` 内容是否一致。

### 11.6 发布候选

- [ ] 所有离线测试通过。
- [ ] 关键真实 Eval 通过。
- [ ] 工作区无未提交文件。
- [ ] 仓库不存在真实 API Key 或未脱敏日志。
- [ ] 从全新目录按 README 可以成功安装和运行。
- [ ] 创建清晰的提交历史。
- [ ] 在确认提交内容后创建 `v0.1.0` 标签。

### 验收标准

- [ ] 评审可以从仓库直接看到自动化测试、Eval、真实演示和安全说明。
- [ ] 项目可以在新环境中按文档复现。
- [ ] README 中每项核心能力都有代码、测试或演示证据。

---

## 12. 全局完成定义

任何阶段只有同时满足以下条件，才可以标记完成：

- [ ] 功能代码已经实现，不只是留下接口或 TODO。
- [ ] 正常路径有自动化测试。
- [ ] 至少一个失败或边界路径有自动化测试。
- [ ] 完整离线测试全部通过。
- [ ] 新增持久化内容经过凭据和隐私检查。
- [ ] 用户可见行为已经更新到文档。
- [ ] 没有把用户已有改动误纳入当前提交。
- [ ] Git Diff 已人工检查。
- [ ] 形成职责单一、消息清晰的提交。
- [ ] 如涉及真实模型，已经保存脱敏结果或兼容性 fixture。

推荐的每阶段收尾流程：

```text
实现
  → 定向测试
  → 完整测试
  → 真实 smoke（适用时）
  → Diff 与敏感信息检查
  → 更新 ROADMAP / README
  → 独立 commit
```

## 13. 优先级与裁剪原则

### P0：考核提交前必须完成

- [ ] 真实模型基线验收。
- [ ] Session 持久化和安全恢复。
- [ ] Diff、修改追踪和冲突检测。
- [ ] 任务验证状态和准确最终报告。
- [ ] API 有限重试、运行预算和统一脱敏。
- [ ] 至少 5 个 Eval 和一个多文件真实演示。
- [ ] CI、文档和全新环境复现。

### P1：显著增强竞争力

- [ ] Session 级安全 Undo。
- [ ] 更细的命令风险分类。
- [ ] 项目入口和测试命令识别。
- [ ] Git 原有改动与 Agent 改动区分。
- [ ] 大文件分页读取和更好的工具结果预算。
- [ ] 跨平台进程树清理。

### P2：当前不优先

- [ ] 多 Agent 编排。
- [ ] GUI 或 IDE 插件。
- [ ] 向量数据库和通用 RAG。
- [ ] MCP 插件生态。
- [ ] 远程后台任务和云端执行。
- [ ] 自动创建 commit、push 或 PR。
- [ ] 自研完整操作系统沙箱。

这些能力并非没有价值，但当前阶段会扩大实现面，削弱核心闭环的完成度和可验证性。只有 P0、P1 已稳定且确有评测收益时才重新评估。

## 14. 建议提交边界

提交名称可在实现时调整，但建议保持以下边界：

```text
test: add live provider baseline fixtures
feat: add persistent and resumable agent sessions
feat: add change tracking, diff preview, and undo
feat: track verification state and structured outcomes
feat: add retries, budgets, command risk, and redaction
feat: improve repository discovery and tool efficiency
test: add coding-agent evaluation harness
ci: add cross-platform offline test workflow
docs: add architecture, evaluation, and demo results
```

不要为了匹配列表强行拆分或合并提交。原则是每个提交能够独立说明、测试和审查，不把无关格式化或临时文件混入功能提交。

## 15. 路线图维护规则

- 开始一个阶段前，先确认上游阶段的验收条件。
- 实现过程中只勾选已经由代码和测试证明的事项。
- 如果设计发生变化，在对应阶段补充原因，不静默删除原目标。
- 新需求先判断属于 P0、P1 还是 P2，再决定是否插入主线。
- 每个阶段完成后更新本文档的勾选状态，并在提交信息或 PR 描述中引用对应章节。
- 如果真实 Eval 表明路线优先级错误，以数据为依据调整顺序，并记录调整原因。

## 16. 当前下一步

1. 完成阶段 B 的真实模型基线验收并保存脱敏记录。
2. 立即进入阶段 C，设计 Session schema、状态机和原子 `SessionStore`。
3. 在编写 `--resume` 之前，先确定工具执行的 `completed` 与 `uncertain` 恢复语义。
4. Session 的正常保存、崩溃恢复和敏感信息测试全部通过后，再进入 ChangeTracker 与 Diff。

